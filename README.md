...
docker buildx build --platform linux/amd64,linux/arm64 -t simon-game:latest .
```
...
docker run --env-file .env -v ./db.sqlite:/app/db.sqlite -p <HOST_PORT>:<CONTAINER_PORT> --name=simon-game -it simon-game:latest

# If using other URI based datastores
docker run --env-file .env -p <HOST_PORT>:<CONTAINER_PORT> --name=simon-game -it simon-game:latest
```
